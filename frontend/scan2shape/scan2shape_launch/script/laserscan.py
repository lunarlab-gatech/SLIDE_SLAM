...
# Script edited for custom point cloud inference
...
class LaserScan:
    ...
    def __init__(self, project=False, H=64, W=1024, fov_up=3.0, fov_down=-25.0, range_threshold=40.0):
    ...
    # Original code to check point cloud. Currently not used
    # -------------------------------------------------------
    #   # check filename is string
    #   if not isinstance(filename, str):
    #     raise TypeError("Filename should be string type, "
    ...
    def do_range_projection(self):
        ...
        # TODO(ankit): Check if this has any unusual effects
        if self.range_threshold > 0.0:
            self.mask = (depth > self.range_threshold)
            ...
            # setting to 1e-10 to avoid divide by 0 errors
            depth[self.mask] = 1e-10
            scan_x[self.mask] = 0
            scan_y[self.mask] = 0
            scan_z[self.mask] = 0
...
class SemLaserScan(LaserScan):
    ...
    def open_label(self, filename):
        ...
        filename = os.path.join(os.path.sep.join(
            filename.split(os.sep)[:-4]), "dataset_ouster_labels.json")
        # check filename is string
        if not isinstance(filename, str):
            raise TypeError("Filename should be string type, "
                            "but was {type}".format(type=str(type(filename))))
    ...
    def set_label(self, label):
        ...
        if label.shape[0] == self.points.shape[0]:
            ...
            # instance id in upper half // This might break it
            self.inst_label = label.astype(int)
        else:
