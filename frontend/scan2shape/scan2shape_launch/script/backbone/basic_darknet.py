...
class Backbone(nn.Module):
    ...
    def __init__(self, params):
        ...
        for s in self.strides:
            current_os *= s
        print("Original Output Stride: ", current_os)

        # make the new stride
        if self.OS > current_os:
            print("Cannot use current stride, ", current_os,
                  " because it is smaller than original ", self.OS)
        else:
            ...
            for i, stride in enumerate(reversed(self.strides), 0):
                if int(current_os) != self.OS:
                    ...
                    if int(current_os) == self.OS:
                        break
            print("New Original Stride: ", int(current_os))
