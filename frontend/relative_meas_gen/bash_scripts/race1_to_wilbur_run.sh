...
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 8; roslaunch sloam decentralized_sloam.launch hostRobot_ID:=0 odom_topic:=/wilbur/odom enable_rviz:=true turn_off_rel_inter_robot_factor:=false param_file:=sloam_CoPeD.yaml" Enter
tmux select-pane -t $SESSION_NAME:2.2
...
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 8; roslaunch sloam decentralized_sloam.launch hostRobot_ID:=2 odom_topic:=/race1/mavros/local_position/odom turn_off_rel_inter_robot_factor:=false param_file:=sloam_CoPeD.yaml" Enter
