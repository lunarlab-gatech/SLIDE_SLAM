...
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 10; cd $BAG_DIR && rosbag play stuff_world-two_jackal_amcl.bag --clock -r $BAG_PLAY_RATE -s 0 --topic /jackal0/odometry/local_filtered /jackal1/odometry/local_filtered /gazebo/model_states /jackal0/odometry/local_filtered:=/robot0/odometry/local_filtered /jackal1/odometry/local_filtered:=/robot1/odometry/local_filtered" Enter
...
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 8; roslaunch sloam decentralized_sloam.launch hostRobot_ID:=0 odom_topic:=/robot0/odometry/local_filtered enable_rviz:=true turn_off_rel_inter_robot_factor:=false  param_file:=sloam_multiUGV.yaml" Enter
tmux select-pane -t $SESSION_NAME:2.2
...
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 8; roslaunch sloam decentralized_sloam.launch hostRobot_ID:=1 odom_topic:=/robot1/odometry/local_filtered turn_off_rel_inter_robot_factor:=false param_file:=sloam_multiUGV.yaml" Enter
