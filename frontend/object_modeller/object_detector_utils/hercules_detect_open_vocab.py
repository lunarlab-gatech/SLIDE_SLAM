...
# import fastsam_wrapper
# import fastsam_params
# from robotdatapy.camera import depthcam_params
...
class sem_detection:
    def __init__(self) -> None:
        ...
        self.model_path = rospack.get_path(
            'object_modeller') + '/models/best.pt'
        ...
        self.odom_topic = rospy.get_param(param_name_prefix + 'odom_topic', '/odom_gt')
        self.visualize = rospy.get_param(param_name_prefix + 'visualize', False)
        self.confidence_threshold = rospy.get_param(param_name_prefix + 'confidence_threshold', 0.00001)
        self.sim = rospy.get_param(param_name_prefix + 'sim', False)
        self.color_fx = rospy.get_param(param_name_prefix + 'color_fx', 376.0)
        self.color_fy = rospy.get_param(param_name_prefix + 'color_fy', 376.0)
        self.color_cx = rospy.get_param(param_name_prefix + 'color_cx', 376.0)
        self.color_cy = rospy.get_param(param_name_prefix + 'color_cy', 240.0)
        ...
        # self.fastsam = fastsam_wrapper.from_params(fastsam_params, depth_cam_params, system_params)
    ...
    def rgb_aligned_depth_odom_callback(self, rgb, aligned_depth, odom):
        ...
        # detections = self.fastsam.run(rospy.Time, odom, color_img, depth_img)
        
        # ADD THIS DEBUG CODE:
        if len(detections[0]) != 0:
            for detection in detections:
                num_obj = detection.boxes.shape[0]
                rospy.loginfo(f"Detected {num_obj} objects")
                for i in range(num_obj):
                    cls_int = int(detection.boxes.cls[i])
                    cls_str = self.yolo.names[cls_int]
                    conf = float(detection.boxes.conf[i])
                    rospy.loginfo(f"  Object {i}: {cls_str} (confidence: {conf:.3f})")
