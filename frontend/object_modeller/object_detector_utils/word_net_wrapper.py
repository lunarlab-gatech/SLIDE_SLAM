...
class WordNetWrapper():
    ...
    def _calculate_word_embeddings(self):
        ...
        if self.use_cupy:
            self.word_features_cupy = np.asarray(self.word_features)
    ...
    def map_embedding_to_words(self, emb: np.ndarray, k: int = 5) -> list[str]:
        ...
        if self.use_cupy:
            similarities = self.word_features_cupy @ np.asarray(emb)
            best_idxs_gpu = np.argsort(similarities)[-top_k:][::-1]
...
def main():
    ...
    wordnetWrapper = WordNetWrapper(["tree"])
    
    wrapped = WordWrapper.from_word("tree")
