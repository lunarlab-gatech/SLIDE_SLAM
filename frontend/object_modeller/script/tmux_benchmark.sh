...
SETUP_ROS_STRING="source ~/sloam_ws/devel/setup.bash; export ROS_MASTER_URI=http://localhost:11311"
...
# # tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 2; rosbag play --clock /home/jiuzl/bags/outdoor_bag/outdoor-car-detection-drifted-5m-shorter-range-second-part-partking-lot-2023-12-05-14-21-56.bag"
# tmux split-window -t $SESSION_NAME
# tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 2; rosparam get /use_sim_time; python3 ./merge_synced_measurements.py" Enter
...
# tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 2; roslaunch object_modeller sync_semantic_measurements.launch odom_topic:=$ODOM_TOPIC robot_name:=''" Enter
# tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 2; python3 ./cylinder_plane_modeller.py --point_cloud_ns $POINT_CLOUD_NS" Enter
# tmux split-window -t $SESSION_NAME
...
# tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 2; roslaunch sloam single_robot_sloam_test_benchmark.launch" Enter
