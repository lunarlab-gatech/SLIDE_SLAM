...
# tmux split-window -h -t $SESSION_NAME
tmux select-pane -t $SESSION_NAME:1.0
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 2; roslaunch object_modeller hercules_rgb_segmentation_open_vocab.launch" Enter
tmux select-pane -t $SESSION_NAME:1.1
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 2; roslaunch object_modeller hercules_sync_semantic_measurements.launch robot_name:=robot0 odom_topic:=/odom_gt" Enter
tmux select-pane -t $SESSION_NAME:1.2
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 2; roslaunch sloam hercules_single_robot_sloam_test.launch enable_rviz:=true" Enter
tmux select-pane -t $SESSION_NAME:1.3
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 2; roslaunch scan2shape_launch hercules_process_cloud_node_rgbd_indoor_open_vocab_with_ns.launch detect_no_seg:=True odom_topic:=/odom_gt robot_name:=robot0" Enter
tmux select-pane -t $SESSION_NAME:1.4
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 2; cd $BAG_DIR && rosbag play drone1_partial.bag --clock -r $BAG_PLAY_RATE -s 0 --topics /odom_gt /cam0 /depth /cam0:=/robot0/camera/color/image_raw /depth:=/robot0/camera/depth/image_rect_raw" Enter
