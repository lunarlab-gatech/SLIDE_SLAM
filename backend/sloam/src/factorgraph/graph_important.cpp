...
void SemanticFactorGraph::addKeyPoseAndBetween(
  ...
  Vector6 noise_vec;
  if(use_odom_cov_in_nav_msg_) { 
    // Use the covariance provided
    for (int i = 0; i < 6; ++i) {
      // Swap from variance XYZRPY to standard deviation RPYXYZ
      noise_vec(i) = std::sqrt(relativeMotionCov[(i + 3) % 6]); 
    }
  } else { 
